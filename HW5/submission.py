...
class ExactInference(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        """ The other car is stationary: H_t = H_t-1 for all t """
        for row in range(self.belief.numRows):
            for col in range(self.belief.numCols):
                # Calculate the distance between the tile and my car
                distance = math.sqrt((util.colToX(col) - agentX) ** 2 + (util.rowToY(row) - agentY) ** 2)
                # Calculate p(e_t|h_t)
                pdf = util.pdf(distance, Const.SONAR_STD, observedDist)
                # Update the probability with respect to the observation
                # P(H_t|e_1:t) = p(e_t|h_t) * P(H_t|e_1:t-1)
                updated_probability = pdf * self.belief.getProb(row, col)
                self.belief.setProb(row, col, updated_probability)
    ...
    def elapseTime(self) -> None:
        ...
        # Declare a new belief for counting accumulated probability
        new_belief = util.Belief(self.belief.numRows, self.belief.numCols, value=0)
        # Accumulating the probability
        for (oldTile, newTile) in self.transProb:
            # P(h_t|e_1:t) * p(h_t+1|h_t)
            reweighted = self.belief.getProb(*oldTile) * self.transProb[(oldTile, newTile)]
            new_belief.addProb(newTile[0], newTile[1], reweighted)
        # Normalize the belief
...
class ParticleFilter(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        # Store reweighted particle distribution to a dictionary
        reweighted = collections.defaultdict(float)
        # Reweight the particle distribution with emission probability p(e_t|h_t)
        for (row, col) in self.particles:
            distance = math.sqrt((util.colToX(col) - agentX) ** 2 + (util.rowToY(row) - agentY) ** 2)
            pdf = util.pdf(distance, Const.SONAR_STD, observedDist)
            # Update the probability
            reweighted[(row, col)] = self.particles[(row, col)] * pdf
        # Store resampled particles to a dictionary
        new_particles = collections.defaultdict(int)
        # Resample particles
        for _ in range(self.NUM_PARTICLES):
            # Distribute to new particles according to the reweighted distribution
    ...
    def elapseTime(self) -> None:
        ...
        # Store new particles to a dictionary
        new_particles = collections.defaultdict(int)
        # Update the particles with transition probability in each tile
        for particle in self.particles:
            for _ in range(self.particles[particle]):
                # Get the next locations for each particle at t+1

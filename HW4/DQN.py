...
class Agent():
    ...
    def learn(self):
        ...
        states, actions, rewards, nextStates, dones = self.buffer.sample(self.batch_size)
        ...
        q_eval = self.evaluate_net(states).gather(1, actions.unsqueeze(1))
        q_next = self.target_net(nextStates).detach()

        for i in range(len(dones)):
            if dones[i]: q_next[i] = 0
        
        q_target = rewards + self.gamma * q_next.max(1).values.unsqueeze(-1)

...
class Agent():
    ...
    def init_bins(self, lower_bound, upper_bound, num_bins):
        ...
        """
        # Begin your code
        arr = np.linspace(lower_bound, upper_bound, num_bins, endpoint = False)
        arr = np.delete(arr, 0)
        return arr
        # End your code
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        print ("( ", self.qtable[state + (action, )], ", ", estimated_optimal_Q, " )\n" )
        if self.qtable[state + (action, )] > estimated_optimal_Q:
